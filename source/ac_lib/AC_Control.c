...
void Speed_Control(void)
{
    ...
    s_speed_right_now = -((ENC_Dateget(ENC2)) * FTM_CONSTANT);
    s_speed_left_now = (ENC_Dateget(ENC3)) * FTM_CONSTANT;

    s_speed_aim = 0.1*data[data_identifier].speed;

    dir_angle = s_dir;    //暂时以占空比的变化当作舵机偏角

    s_speed_aim_left = s_speed_aim*(1-data[data_identifier].speedkl*0.01*SIZE_CONSTANT*tan(dir_angle));
    s_speed_aim_right = s_speed_aim*(1+data[data_identifier].speedkr*0.01*SIZE_CONSTANT*tan(dir_angle));
...
//            Ftm_PWM_Change(FTM0, kFTM_Chnl_3, 20000, 0);
    ...
    else if (data[data_identifier].mode == 0)
    {
    	if (Delayed_departure_flag == 0 || Stop_Car_flag == 2)
		{
//			Ftm_PWM_Change(FTM0, kFTM_Chnl_0, 20000, 0);
//			Ftm_PWM_Change(FTM0, kFTM_Chnl_1, 20000, 0);
//			Ftm_PWM_Change(FTM0, kFTM_Chnl_2, 20000, 0);
//			Ftm_PWM_Change(FTM0, kFTM_Chnl_3, 20000, 0);
		...
		else if (Delayed_departure_flag_1 == 0)
		{
			Speed_Judge(s_speed_left, s_speed_right);
		}
    }
